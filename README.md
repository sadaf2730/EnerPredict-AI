...
## 11. Hackathon Demonstration Flow (3-Minute Script)

1. **Login / Register**: Sign in to the platform.
2. **Select Facility**: Choose **Commercial Office**.
3. **Enter Operational Inputs**:
   - Employees: 137, Computers: 96, ACs: 7 (6.5 hrs/day), Temperature: 31.4°C, Previous Bill: ₹72,000.
4. **Generate Prediction**: Click *Predict My Energy Demand*.
5. **Inspect Dashboard**:
   - Review ML Daily Consumption (~390 kWh), Peak Demand (~43 kW), and Peak Window (`13:00 - 16:00`).
   - Examine Recharts 24-Hour Demand Curve and Appliance Breakdown Donut.
   - Review personalized recommendations with estimated ₹/month savings.
6. **Launch What-If Simulator**:
   - Click **🔮 Try What-If Simulator**.
   - Modify parameters: Adjust AC units from `7 → 5` and AC hours from `6.5 → 5.0h`.
   - Click **Simulate Change**.
7. **Evaluate Before vs After**:
   - Observe actual reduction calculated by the ML model.
   - Review Dual-Line Recharts overlay and estimated potential monthly savings (~₹2,400/month).
...
> *It goes beyond simple descriptive charts by integrating facility-specific machine learning predictions with an active What-If Simulator. Users don't just see what they consumed—they can test operational changes and immediately see how the trained model forecasts demand shifts and monthly cost reductions.*

### Q2: What are the unique inputs?
> *Users enter operational parameters they actually know (number of ACs, hours used, staff count, vehicle charging, weather temperature) rather than electrical engineering figures like connected load in kVA or power factor.*

### Q3: Why Machine Learning instead of a simple formula?
> *Real electrical consumption involves complex non-linear interactions—ambient weather degrades AC cooling efficiency, occupancy non-linearly affects internal heat gain, and coincident equipment timing shifts peak windows. The Gradient Boosting model captures these multi-variable interactions.*

### Q4: How do you know the model actually works?
> *The model was trained on 2 years of synthetic operational data using chronological holdout validation, achieving $R^2 = 0.9993$ on daily consumption and $R^2 = 0.9997$ on peak demand. Sensitivity testing proves that changing inputs (such as increasing/decreasing AC hours) produces consistent, physical responses from the model.*

### Q5: Can a new user use it?
> *Yes. The pipeline receives completely new user inputs, applies feature engineering in real-time, and runs inference through the saved model artifacts. It does not look up dataset rows.*

### Q6: Are predictions or savings hardcoded?
> *No. Every simulation makes a live HTTP request to FastAPI and executes `model.pkl`. If the model is stopped, the backend explicitly returns HTTP 503 and refuses to produce fake results.*
...
### Known Limitations
- The model was trained on synthetic data generated from physics-based cooling and equipment equations. Real-world utility bills may deviate based on building insulation, inverter technology, and regional tariff slab tiers.
- Predictions represent typical seasonal operational days; localized grid outages or severe weather anomalies are not simulated.

### Future Scope
- Integration with smart IoT energy meters and home automation (Home Assistant, Tuya).
- Automated Time-of-Day (ToD) tariff optimization to schedule high-power loads during subsidized solar grid hours.
- Computer vision module for scanning paper utility bills to pre-fill baseline consumption history.
